...
app.controller('BeaconController', ['$scope', '$rootScope', '$ionicPlatform', '$cordovaBeacon', '$ionicModal', 'BeaconUsersCountService', function($scope, $rootScope, $ionicPlatform, $cordovaBeacon, $ionicModal, BeaconUsersCountService) {

  BeaconUsersCountService.getUsers().then(function(users) {
    $scope.count = users;
  });

  BeaconUsersCountService.updateUsers().then(function(users) {
    $scope.newcount = users;
  });
  ...
  $scope.closeBeaconModal = function() {
    $scope.modal.hide();
  };

  // Open the login modal
  $scope.openBeaconModal = function() {
    $scope.modal.show();
  };

  $scope.beacons = {};

  // Make cordovaBeacon available for build!!!!!!!
  /*
      $ionicPlatform.ready(function () {
          ...
          $rootScope.$on("$cordovaBeacon:didRangeBeaconsInRegion", function (event, pluginResult) {
              ...
              for (var i = 0; i < pluginResult.beacons.length; i++) {
                  ...
                  if ($scope.beacons[uniqueBeaconKey] == 'UNIQUE_BEACON_KEY') {
                      openBeaconModal();
                  }
              }
              $scope.$apply();
